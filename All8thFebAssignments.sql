...
FROM inventories INNER JOIN products USING(product_id)
ORDER BY quantity DESC  
OFFSET 10 ROWS  -- skips first 10 row and returns next ten rows
...
(SELECT * FROM product_categories) USING(category_id)
ORDER BY product_name;

--TO BE ASKED AS DOUBT
SELECT category_name, product_name
FROM products p,
...
FROM products p,
LATERAL(SELECT *FROM product_categories c
WHERE c.category_id = p.category_id)
ORDER BY product_name;
