...
--Q1)Get a list of latest order IDs for all customers by using the max function on Order_ID column.
SELECT MAX(orderid), customerid FROM orders GROUP BY customerid;  

--Q2) Find suppliers who sell more than one product to Northwind Trader.

SELECT Suppliers.CompanyName AS SUPPLIER,  
COUNT(Products.ProductID) AS Products
...
--14) Sales by Category: For each category, we get the list of products sold and the total sales amount.
SELECT c.categoryid,c.categoryname,p.productname
FROM categories c
INNER JOIN products p
ON c.categoryid=p.categoryid;

--15) Displays products(productname,unitprice) who?s price is greater than avg(price)
...
--16) Display product(productname), customers(companyname), orders(orderyear)
...
--17) Display Supplier Continent wise sum of unitinstock.
...
--18) Display top 10 expensive products
...
--19) Display customer supplier by city
